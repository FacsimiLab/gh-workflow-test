...
{% set ordered_types = [
    "breaking", "feature", "fix", "perf", "documentation", "chore",
    "ci", "refactor", "style", "build", "unknown"
] %}
...
{# ========== UNRELEASED SECTION ========== #}
{% set unreleased_has_commits = false %}
{% for type_ in ordered_types %}
  {% set commits = context.history.unreleased[type_] %}
  {% if commits %}
    {% for commit in commits %}
      {% set message = commit.message.rstrip() %}
      {% if not ("dependabot" in message or "semantic-release" in message or "update pre-commit hooks" in message) %}
        {% set unreleased_has_commits = true %}
      {% endif %}
    {% endfor %}
  {% endif %}
{% endfor %}

{% if unreleased_has_commits %}
## Unreleased

{% for type_ in ordered_types %}
  {% set commits = context.history.unreleased[type_] %}
  {% if commits %}
    {% set relevant_commits = [] %}
    {% for commit in commits %}
      {% if not ("dependabot" in commit.message or "semantic-release" in commit.message or "update pre-commit hooks" in commit.message) %}
        {% do relevant_commits.append(commit) %}
      {% endif %}
    {% endfor %}
    {% if relevant_commits | length > 0 %}
